...
\\取一个包住内表面的高斯面\\
\therefore \varepsilon_0 \oiint \vec E \cdot d \vec A
\xlongequal{Gauss} \sum_{ins} (q_0 + q')
\xlongequal{上式} \sum_{ins}q_0 - \oiint \vec P \cdot d \vec A\\
\therefore \oiint (\varepsilon_0 \vec E + \vec P) \cdot d \vec A
= \sum_{ins}q_0\\
...
$$
磁偶极子在磁场中所受力矩：\tau = \mu B sin<\vec \mu, \vec B>
\\
...
### Ch31 Electromagnetic-Induction
...
$$
\varepsilon = \oint \vec E \cdot d \vec s = -\frac{d\Phi _B}{dt} 【s为长度量】
$$
...
$$
此外，到$tau$时的电流还是0.63i和0.37i

#### 磁场能量

对上述LR电路，有$\varepsilon i = i^2 R + Li\frac{di}{dt}$，故$U_B = \int_0^i Lidi = \frac12Li^2$

#### 互感

> 在变化磁场中两个线圈之间存在互感：$\Phi_{12} = M_{12}i_1 \\ \Phi_{21} = M_{21}i_2 \\ $
> 互感系数：$M = M_{12} = M_{21} = K\sqrt{L_1L_2}$

$K\in[0,1]，平行嵌套时为1，垂直时为0$
...
==[Chap.31-2.pdf](~/Documents/大二上/普物2_14/WYW课件/Chap.31-2.pdf)和[Chap.31-2.pdf](~/Documents/大二上/普物2_14/WYW课件/Chap.31-2.pdf)后有关于电场与磁场的对比==