...
距圆导线圆心L处：B= \frac{\mu i R^2}{2(L^2 + R^2)^\frac 3 2}\\
(用dB = \frac{\mu_0}{4\pi} \frac{i\ ds\ sin\theta}{r^2}的定义而不是用圆环的结论-FMH's\ Page\ 27,\ WYW的没有)
$$
...
$$
\mathop\oint_L \vec B \cdot d\vec l = \mu_0 \Sigma i(\forall surface\ binded\ by\ Amper\_Loop)
\\
右式正方向同\vec L(\vec n)
\\
