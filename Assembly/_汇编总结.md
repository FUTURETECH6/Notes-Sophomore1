...
**rol, ror, rcl, rcr**(c表示carry)：$\begin{cases};\rm ROL: 循环左移, 高位到低位并送 CF\\;\rm ROR: 循环右移, 低位到高位并送 CF\\;\rm RCL: 循环左移, 进位值(原CF)到低位, 高位进 CF\\;\rm RCR: 循环右移, 进位值(原CF)到高位, 低位进 CF\end{cases}{}$
