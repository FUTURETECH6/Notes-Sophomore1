...
设协方差矩阵为C，行列式为|C|，伴随矩阵为C^*^，则$C^{-1} = \frac{C^*}{|C|} = ?$

#### n元正态分布的四条性质

1. n元正态分布任意k元子向量服从k元正态分布
   1. 特别的，若每个元素服从1元正态分布，X服从n元分布（充要）
2. 线性组合服从正态分布
3. 线性变换也服从正态分布
4. 相互独立 $\Leftrightarrow$ 两两不相关 $\Leftrightarrow$ 协方差为对角矩阵
...
对于随机变量序列X~1~,X~2~,...，若$\exist c, s.t.\forall \varepsilon > 0, \lim_{n\rightarrow +\infty}P\{|X_n - c| \geqslant 0 \} = 0$，则称{T~n~,n>=1}依概率收敛于c
...
特殊的，若$X_n \xrightarrow{P} a$，f在a连续，则$f(X_n) \xrightarrow{P} f(a)$
...
设随机变量X的k阶矩存在(k>=1)，则$\forall \varepsilon > 0$，有$\displaystyle P(|Y| \geqslant \varepsilon) \leqslant \frac{E(|Y|^k)}{\varepsilon^k}$

Proof: 取$Z = \left \{ \begin{array}{cc}
    \varepsilon, & |Y| \geqslant \varepsilon
    \\
    0, & |Y| < \varepsilon
\end{array} \right. \Rightarrow \displaystyle P(|Y| \geqslant \varepsilon) \xlongequal{E(|Z|^k=\varepsilon^kP(|Y| \geqslant \varepsilon)} \frac{E(|Z|^k)}{\varepsilon^k}\leqslant \frac{E(|Y|^k)}{\varepsilon^k}$

#### 切比雪夫不等式

对$\forall \varepsilon>0$，都有$P(|X-E(X)| \geqslant \varepsilon) \leqslant \displaystyle \frac{Var(X)}{\varepsilon^2}$

Proof: 取马尔科夫中 X=X-E(X)，k=2
...
X~1~, X~2~, ...,X~n~相互独立，期望均为μ，方差相同或有共同的上界，则$\displaystyle \frac1n \sum_{i=1}^n X_i \xrightarrow{P} \mu$

#### 辛钦大数定理

X~1~, X~2~, ...,X~n~独立同分布，期望均为μ，则$\displaystyle \frac1n \sum_{i=1}^n X_i \xrightarrow{P} \mu$

**推论**：X~1~, X~2~, ...,X~n~独立同分布，若h(x)为连续函数，E(h(X~i~))=a存在，则$\displaystyle \frac1n \sum_{i=1}^n X_i \xrightarrow{P} a$

#### 贝努里大数定理

n~A~~B(n,p)，则$\displaystyle \frac{n_A}n \xrightarrow{P} p$
