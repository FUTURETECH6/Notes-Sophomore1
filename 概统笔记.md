...
| N(μ,σ^2^) | $f(x) = \frac{1}{\sqrt{2\pi}\sigma} e^{-\frac{(x-\mu)^2}{2\sigma^2}}$<br />F(x): N/A | $\mu$               | $\sigma^2$              |
...
> $\hat\theta_1, \hat\theta_2$是θ的两个**无偏估计**(Premise)，若$\forall \theta \in \Theta, Var(\hat\theta_1) \leqslant Var(\hat\theta_2)$，且$\exist \theta \in \Theta$使等号成立，则称$\hat\theta_1$比$\hat\theta_2$有效
...
> 
> $\hat\theta_1, \hat\theta_2$是θ的两个无偏估计量，若$\forall \theta \in \Theta, Mse(\hat\theta_1) \leqslant Mse(\hat\theta_2)$，且$\exist \theta \in \Theta$使等号成立，则称$\hat\theta_1$优于$\hat\theta_2$
...
> 若对$\forall \theta \in \Theta$，当$n\rightarrow +\infty$，$\hat\theta \xrightarrow{P} \theta$，即$\forall \varepsilon > 0, \lim_{n\rightarrow+\infty} P(|\hat\theta-\theta| \geqslant \varepsilon) = 0$，则称$\hat\theta$为θ的相合估计量或一致估计量
...
3. 分离参数θ，转化不等式：$a<G(X_1,X_2,...,X_n;\theta)<b \Longrightarrow \hat\theta_L < \theta < \hat\theta_U$
   1. 连续离散找置信上下限操作同上
4. ==由于ab的不唯一性，上下街也是不唯一的，因此习惯上让ab满足$P_\theta(G(X_1,X_2,...,X_n;\theta) \leqslant a) = P_\theta(G(X_1,X_2,...,X_n;\theta) \leqslant b) = \frac{\alpha}{2}{}$==
...
* μ~1~-μ~2~的枢轴量：$\left\{ \begin{array}{ll} \frac{(\overline{X}-\overline{Y})-(\mu_1-\mu_2)}{\sqrt{\frac{\sigma_1^2}{n_1}+\frac{\sigma_2^2}{n_2}}} \sim N(0,1)  & (\sigma_1,\sigma^2已知) \\ \frac{(\overline{X}-\overline{Y})-(\mu_1-\mu_2)}{S_w\sqrt{\frac{1}{n_1}+\frac{1}{n_2}}} \sim t(n_1+n_2-2),S_w = \sqrt{{\frac{(n_1-1)S_1^2 + (n_2-1)S_2^2}{n_1+n_2-2}}} & (\sigma_1^2=\sigma_2^2未知) \end{array}\right.$
* σ~1~^2^/σ~2~^2^的枢轴量：$\begin{array}{ll}\frac{(n-1)S^2}{\sigma^2} \sim \chi^2(n-1) & (\mu_1,\mu_2未知)\end{array}{}$

Ex. 

[第7章 参数估计.pdf](file:///Users/ulysses/Documents/大二上/概统_152/课件pdf/第7章-参数估计.pdf) 的slide84

Key: Find the mapping from Xbar to the given random variable. And from the bounds of the given random variable and the inverse **mapping** derive the bounds of Xbar.
...
4. 由于μ的枢轴量的分布均是关于y轴对称的(N,t)，所以才有$z_{1-\alpha} = -z_\alpha$
